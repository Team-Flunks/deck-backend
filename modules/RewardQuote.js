...
//=============== Route Function ===============
const rewardRequestQuote = async function (req, res){
  ...
  async function rewardAPI(user) {
    ...
    const reward = new UserInfo.QuoteModel({quote: ''});
    ...
    try {
      ...
      reward.quote = infoIn.data[0].q;
      reward.author = infoIn.data[0].a;
...
//=============== Reward Creation ===============
    ...
    if (person[0].tokenCount < 1 ){
      ...
      person[0].rewardQuotes.push(reward);
...
module.exports = rewardRequestQuote;